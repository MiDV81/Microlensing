...
def calculate_coefficients(zeta, z_m1, m_t, m_d):
    """
    ...
    c5 = z_m1**2 - zeta_conj**2
    
    c4 = zeta*zeta_conj**2 - zeta*z_m1**2 - m_t*zeta_conj + m_d*z_m1
    
    c3 = 2*m_t*zeta*zeta_conj + 2*zeta_conj**2*z_m1**2 - 2*m_d*zeta_conj*z_m1 - 2*z_m1**4
    
    c2 = zeta*(-2*zeta_conj**2*z_m1**2 + 2*m_d*zeta_conj*z_m1 + m_t**2 + 2*z_m1**4) \
         - m_t*m_d*z_m1 - 2*m_d*z_m1**3
    
    c1 = -2*m_t*zeta*zeta_conj*z_m1**2 + 2*m_t*m_d*zeta*z_m1 \
         - zeta_conj**2*z_m1**4 + 2*m_d*zeta_conj*z_m1**3 \
         - m_d**2*z_m1**2 + z_m1**6 - m_t**2*z_m1**2
    
    c0 = (zeta_conj**2*z_m1**4 - 2*m_d*zeta_conj*z_m1**3 + m_d**2*z_m1**2 - z_m1**6)*zeta \
         + (m_t*zeta_conj*z_m1**4 - m_t*m_d*z_m1**3 + m_d*z_m1**5)
...
# Example usage:
def find_image_positions(zeta, z_m1, m_t, m_d):
    """
    ...
    m1 = (m_t + m_d)/2
    m2 = (m_t - m_d)/2
    z_m2 = -z_m1  # Second lens position
    
    # Get all potential solutions
    coeffs = calculate_coefficients(zeta, z_m1, m_t, m_d)
    solutions = solve_complex_polynomial(coeffs)
    
    # Filter valid solutions
    valid_solutions = [z for z in solutions if check_lens_equation(z, zeta, z_m1, z_m2, m1, m2)]
